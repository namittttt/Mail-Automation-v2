...
# ─────────────────────────────────────────────
# Self-healing helper: package installed but config missing
# ─────────────────────────────────────────────
# If a config directory got deleted by hand (e.g. while testing/
# resetting state) but dpkg still thinks the package is fully
# installed, a plain "apt install" is a no-op -- it never recreates
# the config, because from dpkg's point of view there's nothing to
# do. This function detects that specific mismatch and forces a
# clean purge + reinstall so the package's default config actually
# gets regenerated, instead of failing with "No such file or
# directory" partway through a later script.
heal_if_config_missing() {
    local config_path="$1"
    shift
    local packages=("$@")
    ...
    if [ "$any_installed" -eq 1 ] && [ ! -e "$config_path" ]; then
        echo "  -> ${packages[0]} is installed but $config_path is missing."
        echo "     Purging and reinstalling to regenerate default config..."
        systemctl stop "${packages[0]}" 2>/dev/null || true
...
heal_if_config_missing /etc/ldap/slapd.d slapd
heal_if_config_missing /etc/dovecot dovecot-core dovecot-imapd dovecot-pop3d dovecot-lmtpd dovecot-ldap dovecot-sieve dovecot-managesieved
heal_if_config_missing /etc/postfix postfix postfix-ldap
...
echo
echo "[3.6/9] Pre-seeding postfix..."
# Same category of bug as slapd: postfix has its own debconf prompts
# (mail server type, system mail name). Under DEBIAN_FRONTEND=
# noninteractive with no answer provided, postinst can skip full
# config generation, leaving /etc/postfix incomplete or minimal
# depending on the Debian release. Pre-seed it the same way.
debconf-set-selections <<EOF
postfix postfix/main_mailer_type select Internet Site
postfix postfix/mailname string mail.$DOMAIN
EOF
