...
echo
echo "[1.5/6] Loading qmail-ldap schema (needed by 07-groups.sh)..."
if ldapsearch -Y EXTERNAL -H ldapi:/// -b cn=schema,cn=config -LLL "(cn={*}qmail)" cn 2>/dev/null | grep -q "cn:.*qmail"; then
    echo "qmail schema already loaded, skipping."
else
    SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
    if [ -f "$SCRIPT_DIR/qmail-schema.ldif" ]; then
        ldapadd -Y EXTERNAL -H ldapi:/// -f "$SCRIPT_DIR/qmail-schema.ldif"
        echo "qmail schema loaded."
    else
        echo "ERROR: qmail-schema.ldif not found next to this script."
        echo "07-groups.sh will fail later without it (it uses the"
        echo "qmailGroup objectClass). Make sure qmail-schema.ldif is"
        echo "committed to the repo alongside the other scripts."
        exit 1
    fi
fi
