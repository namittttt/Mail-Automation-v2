...
# python3-systemd is needed to read the journal directly (postfix-sasl
# jail below). Already installed by 01A-install.sh, but harmless to
# ensure here too in case this script runs on an older install.
...
filter       = postfix-sasl
# NOTE: this box is journald-first (Debian 13) -- Postfix logs to the
# journal, not a flat /var/log/mail.log, unless rsyslog is separately
# installed and configured to write one. Reading the journal directly
# is robust regardless of whether rsyslog exists.
backend      = systemd
...
# Dovecot routes ALL Info-level messages there (every login success/
# failure included) -- NOT to log_path (/var/log/dovecot.log), which
# only gets Warning/Error/Fatal. Watching dovecot.log here means this
# jail would NEVER see an auth failure and could never ban anyone.
...
# NOTE: switched from /var/log/apache2/roundcube-error.log to nginx's
# path -- this server now runs Roundcube behind nginx (see
# 05-roundcube.sh), and Apache is stopped/disabled. If this still
# points at the apache2 log, the file stops being written to at all
# and this jail silently goes dead, exactly like the dovecot bug above.
...
echo "========================================"
echo
echo " Fail2ban jails   : postfix-sasl (journald), dovecot (dovecot-info.log), roundcube-auth (nginx)"
