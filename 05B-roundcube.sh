...
# Plain HTTP -> redirect to HTTPS. Without this, the webmail login page
# and password travel completely unencrypted over the network (worse
# than the self-signed cert warning below -- that's still encrypted,
...
server {
    ...
    # Reusing the same self-signed cert already used for IMAP/SMTP TLS.
    # Browsers will show a one-time trust warning (same as any
    # self-signed cert) -- expected for a private/local mail server
    # with no public domain to get a real Let's Encrypt cert for.
